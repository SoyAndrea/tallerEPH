...
Vamos multiplicar por 100 el valor de la tasa, el cual redondeamos a un decimal
...
tabla_3 <- tabla_3 |> 
  mutate(ano_trimestre = paste(ANO4,TRIMESTRE)) |> # creamos una 
  select(-c(ANO4,TRIMESTRE)) |> 
  pivot_wider(names_from = c("ano_trimestre"), values_from = Valor) |> 
  relocate("2023 1",.after = "2022 4" ) 
...
# poner a lo largo la tabla_3 armar variables de resumen ecolución para cada tasa y guardarlas como listas siguiendo este ej. 

 # dplyr::group_by(cyl) %>%
 #    # must end up with list of data for each row in the input dataframe
 #    dplyr::summarize(mpg_data = list(mpg)) %>%

evolucion <- tabla_3 %>%
  pivot_longer(cols = -Tasas, names_to = "Trimestre", values_to = "Valor") %>%
  arrange(Tasas, Trimestre) |> 
  group_by(Tasas) %>%
  summarize(Evolucion = list(Valor))

tabla_5 <- left_join(tabla_3,evolucion) 

tabla_5 |> 
   gt() %>%
   gt_plt_sparkline(Evolucion)
